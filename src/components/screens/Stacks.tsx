  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button className="btn btn-primary" style={{ flex: "none" }} onClick={() => setDraft(emptyStack())}>{t.common.new}</button>
      </div>

      {state.stacks.length === 0 && (
        <Empty title={t.stacks.noneTitle} body={t.stacks.noneBody} />
      )}
      ...
      {state.stacks.map((k) => (
        <button key={k.id} className="card p-5 text-left w-full" style={{ cursor: "pointer" }}
          ...
          <p className="display" style={{ fontSize: 21, lineHeight: 1.35 }}>
            {t.stacks.afterI}{" "}
            <span style={{ borderBottom: "1.5px solid var(--accent)" }}>{label(k.triggerHabitId, k.triggerText)}</span>
            {t.stacks.iWill}{" "}
            <span style={{ borderBottom: "1.5px solid var(--accent)" }}>{label(k.newHabitId, k.newText)}</span>
            {t.stacks.period}
          </p>
          {(k.time || k.location) && (
      ...
      {draft && (
        <Sheet
          open onClose={() => setDraft(null)} title={t.stacks.title}
          footer={
            <>
              {isExisting(draft) && (
                <button className="btn btn-danger mr-auto"
                  onClick={() => { actions.deleteStack(draft.id); setDraft(null); }}>{t.common.delete}</button>
              )}
              ...
              <button className="btn btn-primary" disabled={!complete(draft)}
                onClick={() => { actions.saveStack(draft); setDraft(null); }}>{t.stacks.saveStack}</button>
            </>
          }
        >
          <Field label={t.stacks.fieldTrigger}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label={t.stacks.fieldTime}>
              <input className="input num" type="time" value={draft.time}
                onChange={(e) => setDraft({ ...draft, time: e.target.value })} />
            </Field>
            <Field label={t.stacks.fieldPlace}>
              <input className="input" placeholder={t.stacks.placePlaceholder} value={draft.location}
